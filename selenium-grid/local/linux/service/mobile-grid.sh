...
# === Hub ===
function start_hub() {
  echo -e "${YELLOW} Starting Selenium Hub...${RESET}"
  java -jar "$SELENIUM_JAR" hub >> "$LOG_DIR/hub.log" 2>&1 &
  echo -e "${GREEN}✅ Selenium Hub started → $LOG_DIR/hub.log${RESET}"
}
...
function start_node() {
  ...
  echo -e "${YELLOW} Starting Node: $name...${RESET}"
  java -jar "$SELENIUM_JAR" node --config "$cfg" >> "$LOG_DIR/node-$name.log" 2>&1 &
  echo -e "${GREEN}✅ Node started → $LOG_DIR/node-$name.log${RESET}"
}
...
function start_appium() {
  ...
  echo -e "${YELLOW} Starting Appium: $name...${RESET}"
  appium --config "$cfg" >> "$LOG_DIR/appium-$name.log" 2>&1 &
  echo -e "${GREEN}✅ Appium started → $LOG_DIR/appium-$name.log${RESET}"
}
